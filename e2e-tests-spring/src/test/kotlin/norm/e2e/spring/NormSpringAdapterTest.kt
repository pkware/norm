...
/**
 * Integration tests proving that Norm's adapter injection works through Spring's DI.
 *
 * These tests verify:
 * - Auto-generated enum adapters ([Mood] → [example.MoodAdapter]) are injected via `@Component`
 * - Auto-generated domain adapters ([EmailAddress]) are injected via `@Component`
...
class NormSpringAdapterTest {
  ...
  @Test
  fun `auto-generated enum adapter decodes through Spring DI`() {
  ...
  @Test
  fun `auto-generated domain adapter decodes through Spring DI`() {
  ...
  @Test
  fun `user-configured jsonb adapter decodes through Spring DI`() {
  ...
  fun `insert round-trip encodes and decodes all adapted types`() {
    ...
    val person = queries.getPersonById(id) { _, name, contactEmail, mood, bio ->
