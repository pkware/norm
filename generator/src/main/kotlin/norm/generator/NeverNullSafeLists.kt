...
internal object NeverNullSafeLists {
  ...
  /**
   * (Left operand type, right operand type) pairs shared identically by the numeric arithmetic
   * operators `+`, `-`, `*`, `/` in [NEVER_NULL_OPERATOR_SIGNATURES].
   */
  private val NUMERIC_ARITHMETIC_TYPE_PAIRS: List<Pair<String, String>> = listOf(
    "int8" to "int8",
    "int8" to "int4",
    "int8" to "int2",
    "float8" to "float8",
  ...
  /** Arithmetic operator symbols that each carry the full [NUMERIC_ARITHMETIC_TYPE_PAIRS] set. */
  private val ARITHMETIC_SYMBOLS: List<String> = listOf("*", "+", "-", "/")

  /**
   * (Left operand type, right operand type) pairs shared identically by the comparison operators
   * `<`, `<=`, `<>`, `=`, `>`, `>=` in [NEVER_NULL_OPERATOR_SIGNATURES].
   */
  private val COMPARISON_TYPE_PAIRS: List<Pair<String, String>> = listOf(
    "int8" to "int8",
    "int8" to "int4",
    "int8" to "int2",
    "bool" to "bool",
  ...
  /** Comparison operator symbols that each carry the full [COMPARISON_TYPE_PAIRS] set. */
  private val COMPARISON_SYMBOLS: List<String> = listOf("<", "<=", "<>", "=", ">", ">=")
  ...
  ) + ARITHMETIC_SYMBOLS.flatMap { symbol ->
  ...
  ) + COMPARISON_SYMBOLS.flatMap { symbol ->
    COMPARISON_TYPE_PAIRS.map { (left, right) -> SafeOperatorSignature(symbol, left, right) }
  } + listOf(
