...
/** Recursion depth guard shared by every recursive walk over a [PgNodeExpression] tree in this package. */
...
internal class NodeTreeNullabilityAnalyzer(
  ...
  private val groupingSetNullExtension =
    GroupingSetNullExtension(isAlwaysNonNull, isNonNullIffFirstArgumentNonNull, isFoldableToConst)
  ...
  fun extractColumnNullability(nodeTreeText: String): List<Boolean> {
    ...
    val groupingKeyExpressions = if (hasGroupingSets) {
      groupingSetNullExtension.groupingKeyExpressions(entries, groupingSortGroupRefs)
    } else {
      emptySet()
    }
  ...
  /**
   * Evaluates whether [entry] is guaranteed non-null, delegating to
   * [GroupingSetNullExtension.forcesNullable] when [hasGroupingSets] is `true` before falling back
   * to ordinary [isNonNull] evaluation.
   */
  private fun isEffectivelyNonNull(
    ...
    if (hasGroupingSets &&
      groupingSetNullExtension.forcesNullable(entry, groupingSortGroupRefs, groupingKeyExpressions)
    ) {
      return false
    }
