...
class PgNodeTreeParserTest {
  ...
  fun `parseTargetList reads the full, unescaped resname of an entry whose alias contains an escaped brace`() {
    ...
    // real value ends — resultName comes back as "k\}" (missing the trailing "x") instead of the
    // correctly-bounded and unescaped "k}x". This is the same brace-counting bug hasGroupingSets
    // hits, just visible through a different accessor: the fields extracted from a wrongly-bounded
    // block are wrong, not just the block boundary.
    val text = """
      {QUERY :targetList (
